...
NonlinearModel::NonlinearModel(double& mi_,  double& Te_, double& B0_, double& E0_ ,
              ...
              tf{tf_}, n0{n0_}, n10{n10_n0_*n0}, Ni{Ni_}, Nj{Nj_} {
                ...
                dy = 2*M_PI*R0/Nj;
                dt = tf/Ni;
                b = dense_vector<complex_number>(spacesteps_number());
...
void NonlinearModel::BuildInitialConditions(){
    ...
    for( int j = 0; j < Nj+1; j++){
      ...
      phi1[1][j] = dphi_decr(n10/n0)  * ( cos(my_decr/R0*y) +i1*sin(my_decr/R0*y) );
  ...
  void NonlinearModel::solver_validation_t0(){
    ...
    std::ofstream space_file("space.txt");
    std::ofstream exact_linear_file("exact_linear.txt");
    std::ofstream solution_linear_file("solution_linear.txt");
    std::ofstream solution_nonlinear_file("solution_nonlinear.txt");
    ...
    for( int j = 0; j < Nj+1; j++){
      y = dy*(1.0-1.0/Nj)*j;
      space[0][j] = y;
      exact_linear[0][j] = dphi_decr(n10/n0)  * ( cos(my_decr/R0*y) +i1*sin(my_decr/R0*y) );
    }
